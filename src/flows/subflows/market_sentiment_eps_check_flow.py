...
from src.tasks.cache_retrieval.market_sentiment_eps_check_cache_retrieval_task import (
    market_sentiment_eps_check_cache_retrieval_task,
)
from src.tasks.common.job_status_task import update_job_status_task
from src.tasks.eps_validation.market_sentiment_eps_check_task import (
...
async def market_sentiment_eps_check_flow(
    ...
    await update_job_status_task(
        job_id,
        JobStatus.RUNNING,
        "Running market sentiment EPS validation analysis",
        "market_sentiment_eps_check_flow",
    )

    # Try to get cached result first
    cached_result = await market_sentiment_eps_check_cache_retrieval_task(
    ...
    if cached_result is not None:
        ...
        await update_job_status_task(
            job_id,
            JobStatus.RUNNING,
            "Market sentiment EPS check completed (cached)",
            "market_sentiment_eps_check_flow",
        )
    ...
    await update_job_status_task(
        job_id, JobStatus.RUNNING, verdict_message, "market_sentiment_eps_check_flow"
    )

    logger.info(
        f"Market sentiment EPS check flow completed for {symbol} in {int(time.time() - start_time)} seconds"
    )
    logger.info(
        f"Validation verdict: {validation_result.sentiment_validation_verdict.value}"
    )
