...
from src.tasks.cache_retrieval.bottom_up_eps_validation_cache_retrieval_task import bottom_up_eps_validation_cache_retrieval_task
...
async def bottom_up_eps_validation_flow(
    ...
    # Update job status
    await update_job_status_task(job_id, JobStatus.RUNNING, "Running bottom-up EPS validation analysis", "bottom_up_eps_validation_flow")

    # Try to get cached result first
    cached_result = await bottom_up_eps_validation_cache_retrieval_task(symbol, force_recompute)
    if cached_result is not None:
        logger.info(f"Returning cached bottom-up EPS validation for {symbol}")
        await update_job_status_task(job_id, JobStatus.RUNNING, "Bottom-up EPS validation completed (cached)", "bottom_up_eps_validation_flow")
    ...
    await update_job_status_task(job_id, JobStatus.RUNNING, verdict_message, "bottom_up_eps_validation_flow")

    logger.info(f"Bottom-up EPS validation flow completed for {symbol} in {int(time.time() - start_time)} seconds")
