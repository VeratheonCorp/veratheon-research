...
async def eps_validation_synthesis_cache_retrieval_task(
    symbol: str, force_recompute: bool = False
) -> Optional[EpsValidationSynthesis]:
    """
    ...
    if force_recompute:
        logger.info(
            f"Skipping cache lookup for EPS validation synthesis: {symbol} (force_recompute=True)"
        )
        return None
    ...
    if cached_report:
        ...
        try:
            ...
            logger.warning(
                f"Failed to reconstruct cached EPS validation synthesis data for {symbol}, falling back to fresh analysis: {str(e)}"
            )
    else:
