...
async def peer_relative_eps_validation_cache_retrieval_task(
    symbol: str, force_recompute: bool = False
) -> Optional[PeerRelativeEpsValidation]:
    """
    ...
    if force_recompute:
        logger.info(
            f"Skipping cache lookup for peer-relative EPS validation: {symbol} (force_recompute=True)"
        )
        return None
    ...
    if cached_report:
        ...
        try:
            ...
            logger.warning(
                f"Failed to reconstruct cached peer-relative EPS validation data for {symbol}, falling back to fresh analysis: {str(e)}"
            )
    else:
