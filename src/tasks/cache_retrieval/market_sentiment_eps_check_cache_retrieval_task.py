...
async def market_sentiment_eps_check_cache_retrieval_task(symbol: str, force_recompute: bool = False) -> Optional[MarketSentimentEpsCheck]:
    """
    ...
    """
    if force_recompute:
        logger.info(f"Skipping cache lookup for market sentiment EPS check: {symbol} (force_recompute=True)")
        return None
    ...
    if cached_report:
        ...
        try:
            ...
            logger.warning(f"Failed to reconstruct cached market sentiment EPS check data for {symbol}, falling back to fresh analysis: {str(e)}")
    else:
